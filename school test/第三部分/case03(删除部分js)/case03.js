...
function inputBlur() {
    ...
    var tips_obj = this.parentNode.nextElementSibling.firstElementChild; // 获取提示信息显示的div元素对象
...
function getRegMsg(name, tips) {
    ...
    switch (name) {
        ...
        case 'pwd':
            ...
            break;
        case 'repwd':
            var pwd = document.getElementsByName('pwd')[0].value; // 获取密码输入框的值
            reg = new RegExp("^" + pwd + "$"); // 使用正则表达式匹配确认密码
